...
function* GenerateMap() {
	...
	var terrains = ["new_alpine_grass_b", "new_alpine_grass_c", "temp_dirt_gravel_plants", "temp_grass_d"];
	...
	terrains = ["temp_dirt_gravel_plants|gaia/ore/temperate_small", "temp_dirt_gravel_plants|gaia/rock/temperate_small"].concat(terrains, terrains);
	...
	textueByHeight.push({
		"upperHeightLimit": heightSeaGroundAdjusted + 2 / 6 * (heightRange.max - heightSeaGroundAdjusted),
		"terrain": ["new_alpine_grass_b", "temp_grass_d", "new_alpine_grass_b", "temp_grass_plants"]
	});

	// Medium level grass
	// var testActor = "actor|geology/decal_stone_medit_a.xml";
	textueByHeight.push({
		"upperHeightLimit": heightSeaGroundAdjusted + 3 / 6 * (heightRange.max - heightSeaGroundAdjusted),
		"terrain": ["temp_grass", "temp_grass_b", "temp_grass_c", "temp_grass_mossy"]
	});

	// Long grass near forest border
	textueByHeight.push({
		"upperHeightLimit": heightSeaGroundAdjusted + 4 / 6 * (heightRange.max - heightSeaGroundAdjusted),
	...
	// Unpassable woods
	textueByHeight.push({
		"upperHeightLimit": heightSeaGroundAdjusted + 6 / 6 * (heightRange.max - heightSeaGroundAdjusted),
		...
		for (let i = 0; i < numPlayers; ++i) {
			...
			for (let index = 0; index < 2; index++) {
				...
				for (let k = 0; k < chickenCount; ++k) {
					let pos = Vector2D.sum([
						playerPosition[i],
						new Vector2D(chickenDistance + Math.floor((Math.random() * 10) + 1), 0).rotate(-uAngle),
						new Vector2D(k + 1 * resourceSpacing, 0).rotate(-uAngle - Math.PI / 2),
						new Vector2D(-0.55 * resourceSpacing * Math.floor(chickenCount / 2), 0).rotate(-uAngle - Math.PI / 2)
					]);

					g_Map.placeEntityPassable("gaia/fauna_chicken", 0, pos, randomAngle());
